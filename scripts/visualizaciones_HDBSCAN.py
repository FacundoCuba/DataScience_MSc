...
"""Visualizador Interactivo GPU-Acelerado (cuML / CUDA) con Clustering HDBSCAN.
...
def project_and_cluster_gpu(X, n_components: int, metric: str = "cosine"):
    """Procesa matrices densas y esparcidas usando SVD en GPU (CuPy) para alta dimensionalidad."""
    ...
    if issparse(X):
        ...
        # 2. SVD esparcida acelerada 100% en GPU
        # k=50 calcula únicamente los 50 componentes principales
        u, s, _ = cupy_svds(X_gpu_sparse, k=50)
        
        # Proyección: Multiplicación de la matriz U por los valores singulares S
        X_gpu = u * s
        
        # Liberar la matriz esparcida de la VRAM inmediatamente
    ...
    # 3. Reducción UMAP sobre las 50 dimensiones densas en VRAM
    reducer = cuUMAP(
    ...
    # 4. Clustering HDBSCAN en GPU
    clusterer = cuHDBSCAN(
        min_cluster_size=20,
        metric="euclidean"
    )
    labels_gpu = clusterer.fit_predict(coords_gpu)

    # Limpieza de VRAM al finalizar la proyección
    del X_gpu
    cp.get_default_memory_pool().free_all_blocks()

    return cp.asnumpy(coords_gpu), cp.asnumpy(labels_gpu)
...
def generate_interactive_plots(ids, y_product, X_k6, X_pt5, X_esm):
    ...
    for idx, (name, X, metric) in enumerate(models, 1):
        print(f"[{time.strftime('%H:%M:%S')}] Ejecutando UMAP 2D + HDBSCAN en GPU para {name}...")
        coords_2d, labels_2d = project_and_cluster_gpu(X, n_components=2, metric=metric)
        ...
        fig_2d.add_trace(
            go.Scattergl(
                ...
                y=coords_2d[:, 1],
                mode='markers',
                text=[f"ID: {p}<br>Prod: {prod}<br>Cluster: {c}" for p, prod, c in zip(ids, y_product, labels_2d)],
                marker=dict(size=2, color=labels_2d, colorscale='Viridis', opacity=0.6),
                name=name
            ), row=1, col=idx
        )

        print(f"[{time.strftime('%H:%M:%S')}] Ejecutando UMAP 3D + HDBSCAN en GPU para {name}...")
        coords_3d, labels_3d = project_and_cluster_gpu(X, n_components=3, metric=metric)
        ...
        fig_3d.add_trace(
            go.Scatter3d(
                ...
                z=coords_3d[:, 2],
                mode='markers',
                text=[f"ID: {p}<br>Prod: {prod}<br>Cluster: {c}" for p, prod, c in zip(ids, y_product, labels_3d)],
                hoverinfo='text',
    ...
    fig_2d.update_layout(
        title="Topología Espacial (UMAP 2D + HDBSCAN)",
        template="plotly_dark",
        height=600,
        showlegend=False
    )
    fig_3d.update_layout(
        title="Espacios Latentes Tridimensionales (UMAP 3D + HDBSCAN)",
        template="plotly_dark",
        height=750,
        showlegend=False
    )
